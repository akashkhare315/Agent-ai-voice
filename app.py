...
st.set_page_config(page_title="VoiceAgent", page_icon="🎙️", layout="wide", initial_sidebar_state="expanded")
...
<div class="header-block">
  <div class="header-title"><span class="accent">Voice</span>Agent</div>
...
with st.sidebar:
    ...
    llm_provider = st.selectbox(
        "LLM Provider",
        ["Groq LLM", "Anthropic Claude", "OpenAI GPT-4", "Ollama (local)"],
        index=0,
    ...
    human_in_loop = st.toggle("Human-in-the-Loop", value=True, help="Require confirmation before file operations")
...
with left_col:
    ...
    input_mode = st.radio("Audio Source", ["Upload Audio File", "Record via Microphone (browser)"],
        horizontal=True, label_visibility="collapsed")

    audio_bytes = None

    if input_mode == "Upload Audio File":
        uploaded = st.file_uploader("Drop an audio file here",
            type=["wav", "mp3", "m4a", "ogg", "webm"], label_visibility="collapsed")
        if uploaded:
        ...
        try:
            ...
            st.warning("⚠ Microphone not supported. Use 'Upload Audio File' or type a command below.")

    with st.expander("✍ Or type a command directly (skip audio)"):
        text_override = st.text_area("Command text",
    ...
    if run_button and (audio_bytes or text_override.strip()):
        ...
        with result_box:
            ...
            with st.spinner("🧠 Classifying intent…"):
                try:
                    intent_data = classify_intent(transcript, provider=llm_provider,
            ...
            st.markdown(f"""<div class="step-card">
              ...
              <div class="step-content"><b>Parameters:</b> {json.dumps(intent_data.get("parameters", {}))}</div>
            </div>""", unsafe_allow_html=True)
            ...
            if needs_confirm:
                st.session_state.pending_action = {"transcript": transcript, "intent_data": intent_data, "llm_provider": llm_provider}
                st.markdown("""<div class="confirm-box">
                  ⚠ This action will create or modify files in the <code>output/</code> folder. Confirm below to proceed.
                </div>""", unsafe_allow_html=True)
            else:
                st.session_state.pending_action = {"transcript": transcript, "intent_data": intent_data,
                    "llm_provider": llm_provider, "auto_approve": True}
    ...
    if st.session_state.pending_action and st.session_state.pending_action.get("auto_approve"):
        ...
        with st.spinner("⚙ Executing…"):
            try:
                exec_result = execute_action(pending["intent_data"], pending["transcript"],
                    output_dir=OUTPUT_DIR, llm_provider=pending["llm_provider"])
            except Exception as e:
        ...
        st.markdown(f"""<div class="result-card {status_cls}">
...
with right_col:
    ...
    if not st.session_state.history:
        ...
        for item in reversed(st.session_state.history):
            intents_html = " ".join([f'<span class="intent-badge-sm">{i}</span>' for i in item["intents"]])
